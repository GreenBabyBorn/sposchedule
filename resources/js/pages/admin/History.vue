...
const { data: histories, isLoading, refetch } = useHistoryQuery(currentPage, rowsPerPage, searchTerm);
...
<template>
    <div class="flex flex-col gap-4">
        ...
        <div class="">
            <DataTable v-model:filters="filters" class="datatable-wrapper" :loading="isLoading"
                v-model:selection="selectedHistories" paginator :rows="rowsPerPage"
                :first="(currentPage - 1) * rowsPerPage" :totalRecords="histories?.meta?.total" lazy
                :globalFilterFields="['action', 'details', 'created_at']" :value="histories?.data" :onPage="(event) => {
