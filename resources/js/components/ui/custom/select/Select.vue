                ...
                >
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        <span
                          :class="cx('optionGroupLabel')"
                          v-bind="ptm('optionGroupLabel')"
                          >{{ getOptionGroupLabel(option.optionGroup) }}</span
                        >
                      </slot>
                    </li>
                    <li
                      ...
                      :class="
                        cx('option', {
                          option,
                          focusedOption: getOptionIndex(i, getItemOptions),
                        })
                      "
                      :style="{
                        height: itemSize ? itemSize + 'px' : undefined,
                      ...
                      v-bind="
                        getPTItemOptions(option, getItemOptions, i, 'option')
                      "
                    ...
                    >
                      <template v-if="checkmark">
                        <CheckIcon
                          ...
                          v-bind="ptm('optionCheckIcon')"
                        />
                        <BlankIcon
                          v-else
                          :class="cx('optionBlankIcon')"
                          v-bind="ptm('optionBlankIcon')"
                        />
                      </template>
                      <slot
                        name="option"
                      ...
                      >
                        <span
                          :class="cx('optionLabel')"
                          v-bind="ptm('optionLabel')"
...
<script>
  ...
  export default {
    ...
    computed: {
      visibleOptions() {
        const options = this.optionGroupLabel
          ? this.flatOptions(this.options)
          : this.options || [];

        if (this.filterValue) {
          const filteredOptions = FilterService.filter(
            options,
            this.searchFields,
            this.filterValue,
          ...
          if (this.optionGroupLabel) {
            ...
            optionGroups.forEach(group => {
              ...
              const filteredItems = groupChildren.filter(item =>
    ...
    methods: {
        ...
        return this.ptm(key, {
          context: {
            ...
            focused:
              ...
              this.getOptionIndex(index, itemOptions),
      ...
      isOptionMatched(option) {
        return (
          ...
          this.getOptionLabel(option)
            ...
            .includes(this.searchValue.toLocaleLowerCase(this.filterLocale)) // Проверка на вхождение
        );
      },
      ...
      findLastOptionIndex() {
        return findLastIndex(this.visibleOptions, option =>
          this.isValidOption(option)
        );
      },
      ...
      findPrevOptionIndex(index) {
        const matchedOptionIndex =
          index > 0
            ? findLastIndex(this.visibleOptions.slice(0, index), option =>
                this.isValidOption(option)
              )
            : -1;
      ...
      scrollInView(index = -1) {
        this.$nextTick(() => {
          const id =
            index !== -1 ? `${this.id}_${index}` : this.focusedOptionId;
